    ...
    try {
      ...
      const contactsFormatted = contacts.map((contact, contactIndex) => {
        const contactFormatted = Object.entries(contact)
          .map(([key]) => {
            const column = Object.entries(columnsMatchingComplete).find(([k, e]) => e === key)
            if (column) {
    ...
    >
      { contacts.length > 0
        ...
        <form onSubmit={importContacts}>
          <p className={styles.countContactsFound}><span>{contacts.length} contacts found</span> in your CSV file.</p>
          { isColumnMissing && 
            <div className={styles.columnsNotMatching}>
              <p>We are unable to detect some of the columns in your CSV.<br />Please connect the missing columns with the correct columns from your file.</p>
              <div className={styles.columnsNotMatchingList}>
                {
                  Object.keys(columnsMatching).map((key) => {
                    if (columnsMatching[key] === null) {
                      return (
                        <div
                          className={styles.columnsNotMatchingItem}
                          key={key}
                        >
                          <p>{capitalize(key.replace(/([A-Z])/g, ' $1').trim())}</p>
                          <select
                            defaultValue=""
                            onChange={(e) => setColumnsMatchingComplete({
