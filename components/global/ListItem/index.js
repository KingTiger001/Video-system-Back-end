...
const CampaignListItem = ({
    ...
    if (campaign.share.thumbnail) {
      ...
      if (getTemplateHasMoreContents(thereIsTemplates)) {
        if (getTemplateHasMoreContents(thereIsTemplates).thumbnail)
          ...
          setThumbnail({
            type: "bg",
            data: getTemplateHasMoreContents(thereIsTemplates).screen.background
              .color,
          });
      }
    }
  ...
  useEffect(() => {
    const askApi = async () => {
      const views = await mainAPI.get(`/campaigns/watches/${campaign._id}`);
  ...
  return (
    <div className={`${styles.root} ${styles.dark}`}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.duration}>{views}</div>
